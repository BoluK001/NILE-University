report 51534718 "prPaySlip2 Taxable1"
{
    DefaultLayout = RDLC;
    RDLCLayout = './prPaySlip2Taxable1.rdl';
    ...
    dataset
    {
        dataitem("HR-Employee"; "HR-Employee")
        {
            ...
            dataitem("prSalary Card"; "prSalary Card")
            {
                ...
                begin
                    ...
                    *****************************************************************************************************/
                    ...
                    PeriodTrans.SETFILTER(PeriodTrans."Transaction Code",'<>%1','PENSNORM');
                    //PeriodTrans.SETTER(PeriodTrans.Frequency,'<>%1',PeriodTrans.Frequency::"2");
                    ...
                    PeriodTrans.SetRange(PeriodTrans."Employee Code", "Employee Code");
                    PeriodTrans.SetRange(PeriodTrans."Payroll Period", SelectedPeriod);
                    PeriodTrans.SetRange(PeriodTrans."Transaction Code", 'TOT-PAYE');
                    if PeriodTrans.Find('-') then
                        repeat
                            TaxPayTillDate := TaxPayTillDate + PeriodTrans.Amount;
                        until PeriodTrans.Next = 0;


                    //Message('%1',TaxPayTillDate);
                    ...
                    PeriodTrans.SetFilter(PeriodTrans."Transaction Name", '<>%1', 'Pension ER');
                    PeriodTrans.SetFilter(PeriodTrans."Transaction Code", '<>%1', 'MONTHRF');
                    ...
                    if PeriodTrans.Find('-') then
                        repeat
                            if (PeriodTrans."Transaction Code" <> 'TXCHRG') and (PeriodTrans."Transaction Code" <> 'NONTAX')
                            and (PeriodTrans."Transaction Code" <> 'PENS') then begin  //PKK  NONTAX
                                if strGrpText <> PeriodTrans."Group Text" then begin
                                    ...
                                    //Message('%1',EVALUATE(TransAmt[ColumnNo,Index],FORMAT(ROUND(PeriodTrans.Amount+StatutoryAmt,0.01))));
                                        ...
                                        if PeriodTrans."Group Order" = 4 then
                                            ...
                                            //ELSE IF (PeriodTrans."Group Order" = 7) AND (PeriodTrans."Sub Group Order" = 4) THEN
                                            // Message('%1...',PeriodTrans."Transaction Name")

                                            else
                                                if PeriodTrans."Transaction Code" = 'NONTAX' then
                                                    Evaluate(TransAmt[ColumnNo, Index], Format(Round(PeriodTrans.Amount - NonTaxable, 0.01)))
                                    ...
                                    if PeriodTrans."Transaction Code" = 'TOT-DED' then
                                        ...
                                        if PeriodTrans."Group Order" = 4 then
                                            ...
                                            if PeriodTrans."Group Order" = 9 then
                                                ...
                                                //ELSE IF (PeriodTrans."Group Order" = 7) AND (PeriodTrans."Sub Group Order" = 4) THEN begin
                                                //Message('%1...',PeriodTrans."Transaction Name")
                                                //EVALUATE(TransAmt[ColumnNo,Index],FORMAT(ROUND(PeriodTrans.Amount-NonTaxable,0.01)))
                    ...
                    */ //PKK - NG
                    ...
                    Index += 1;
                    Trans[ColumnNo, Index] := 'Bank:';
                    Evaluate(TransAmt[ColumnNo, Index], Format(strBank));  //jj270422 I changed from strbranch

                    //Account No:
                    Index += 1;
                    Trans[ColumnNo, Index] := 'Account No';
                    Evaluate(TransAmt[ColumnNo, Index], Format(strBankNo));  //jj270422 I changed from strAccountNo
            ...
            trigger OnPreDataItem()
            begin
                //Message('%1',"HR-Employee"."Payroll Filter Group");
                /* hrEmp.Get("HR-Employee".GetFilter("No."));
                UserSetup.Get(UserId); */ //Biyi190522
                hrEmp.SetRange("No.");
                hrEmp.FindSet();
                UserSetup.Get(UserId); // Biyi190522
                //if UserSetup."PayRoll Cent. Filter" <> hrEmp."Payroll Filter Group" then
                // CurrReport.Quit;
            end;
        }
    }
    ...
    var
        ...
        dept: Text[100];  //jj270422 I increased from 30
