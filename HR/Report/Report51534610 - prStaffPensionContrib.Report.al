report 51534610 "prStaff Pension Contrib"
{
    DefaultLayout = RDLC;
    RDLCLayout = './prStaffPensionContrib.rdlc';
    ...
    dataset
    {
        dataitem("prSalary Card"; "prSalary Card")
        {
            ...
            begin
                ...
                ObjHREmp.Reset;
                ObjHREmp.SetRange(ObjHREmp."No.", "Employee Code");
                if ObjHREmp.Find('-') then begin
                    EmployeeName := ObjHREmp."First Name" + ' ' + ObjHREmp."Middle Name" + ' ' + ObjHREmp."Last Name";
                    PINNo := ObjHREmp."PIN Number";
                    Location := ObjHREmp."Global Dimension 2 Code";
                    PensionH := ObjHREmp."Pension Fund Administrator";
                    ...
                    PINNo:=objEmp."PIN No.";
                    Location:=objEmp."Location/Division Code";
                ...
                if PeriodTrans.Find('-') then begin
                    SelfContrib := (PeriodTrans.Amount / 3 * 1);
                    //MESSAGE('CHECKED1');
                ...
                //PeriodTrans.SETRANGE(PeriodTrans."Transaction Name",'PENSION');
                PeriodTrans.SetRange(PeriodTrans."Company Deduction", false);  //dennis
                //PeriodTrans.SETRANGE(PeriodTrans.Pension,FALSE);  //dennis
                PeriodTrans.SetFilter(PeriodTrans."Transaction Code", '%1|%2|%3', 'PENSNOR', 'PENS-CALC', 'PENS-SA');   //SelfContribCode
                if PeriodTrans.Find('-') then begin
                    //SelfContrib:=(PeriodTrans.Amount/3*1);
                    //CompanyContrib:=PeriodTrans.Amount;
                ...
                PeriodTrans.SetRange(PeriodTrans."Company Deduction", false);  //dennis
                //PeriodTrans.SETRANGE(PeriodTrans.Pension,FALSE);  //dennis
                PeriodTrans.SetFilter(PeriodTrans."Transaction Code", '%1|%2|%3', 'PENSNORM', 'PENSCALCER', 'PENS-SA');   //SelfContribCode
                if PeriodTrans.Find('-') then begin
                    PenER := PeriodTrans.Amount;
               ...
               BEGIN
               CompanyContrib:=(prEmployerContrib.Amount/3*2);
               END;
                ...
                */
                ...
                objTransCode.Reset;
                objTransCode.SetRange(objTransCode."Transaction Code", 'AVC');
                ...
                CummContrib := SelfContrib + PenER + VoluntaryCont;
